...
class Review:
    ...
    def build(
        ...
        additional_attrs_per_system = eval_packages_per_system(
            dict.fromkeys(self.systems, prefixed_additional),
            self.build_config,
        )
        combined_attrs_per_system = {
            system: list(
                (
                    attrs_per_system.get(system, {})
                    | additional_attrs_per_system.get(system, {})
                ).values()
            )
            for system in attrs_per_system.keys() | self.systems
        }
